...
const Sidebar = () => {
  ...
  const [input, setInput] = useState('') // ✅ false → '' (string)


  const navigate = useNavigate();

  // ✅ user.fullName के लिए safety जोड़ी गई
      ...
      <div className='flex flex-col'> {/* ✅ flex-flex-col → flex flex-col */}
        {filteredUsers.map((user, index) => (
          <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev => 
            ({...prev, [user._id]:0})) }}
            key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:test-sm
